 ...
 * 评分（Mock + 接口预留）
 * ====================================================================
 * Prompt 已按教育部高考语文作文评分标准写好，接入真实星火时
 * 把这段 prompt 直接发过去即可。
 * -------------------------------------------------------------------- */
const SCORE_PROMPT = `你是一位资深高考语文阅卷教师。请按 2025 年高考语文作文评分标准（基础等级 40 分 + 发展等级 20 分 = 总分 60 分）严格评分。

【评分维度】
一、基础等级（40 分）：
  ① 切合题意（10 分）：准确把握题目核心意旨，无跑题偏题
  ② 中心突出 + 内容充实（10 分）：观点明确，论据充实
  ③ 结构严谨 + 语言流畅（10 分）：逻辑清晰，文笔通顺
  ④ 字数 + 书写规范（10 分）：符合 800 字以上、错别字少

二、发展等级（20 分）：
  ① 深刻（5 分）：透过现象看本质，辩证分析
  ② 丰富（5 分）：材料丰富、论据多样
  ③ 有文采（5 分）：用词贴切、句式灵活、善用修辞
  ④ 有创意（5 分）：见解新颖、构思新巧

【任务】
1. 仔细阅读下面题目（材料）和作文正文
2. 按上述标准打分，输出基础等级和发展等级的小分与理由
3. 给出 3-5 条具体的「修改升格建议」，每条建议必须：
   - 指出具体问题（如：素材与论点脱节 / 辩证深度不足 / 文采欠佳等）
   - 给出可直接套用的【AI 生成】示例句 / 段落（不少于 60 字，要能直接放进作文）
...
{
  "total": <总分，整数 0-60>,
  "basic": <基础等级得分，整数 0-40>,
  "basicReason": "<基础等级的详细评分理由，不少于 80 字>",
  "develop": <发展等级得分，整数 0-20>,
  "developReason": "<发展等级的详细评分理由，不少于 80 字>",
